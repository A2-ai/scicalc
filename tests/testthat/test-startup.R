test_that("startup message lists categorical configs under their own section", {
  ...
  expect_match(all, "scicalc.racen_config:")
  expect_match(all, "white -> 10", fixed = TRUE)
  expect_match(all, "black -> 20", fixed = TRUE)
  expect_match(all, "asian -> 3", fixed = TRUE)
  expect_match(all, "other -> 6", fixed = TRUE)
