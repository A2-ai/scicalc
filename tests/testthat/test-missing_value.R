...
test_that("is_missing_value works on units objects", {
  x <- units::set_units(c(1, -999, 3), "d", mode = "standard")
  expect_equal(is_missing_value(x), c(FALSE, TRUE, FALSE))
})
...
test_that("is_missing_value can be used to filter a units column", {
  df <- data.frame(NTFD = c(0, 1, -999, 2))
  df$NTFD <- units::set_units(df$NTFD, "d", mode = "standard")
  out <- dplyr::filter(df, !is_missing_value(NTFD))
  expect_equal(as.numeric(out$NTFD), c(0, 1, 2))
})
