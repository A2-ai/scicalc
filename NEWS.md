...
* Added an assembly provenance audit. `audit_script()` re-runs an assembly script (`.R`/`.qmd`/`.Rmd`, converting Quarto/R Markdown via `knitr::purl()` then running it with `callr::rscript()`) in a fresh subprocess with capture enabled, recording one clean provenance run — every unit conversion plus input/spec/output blake3 hashes — to `.scicalc-logs/<name>.audit.log` (via `log4r`). `scicalc_audit(name)` reads it back as a tidy tibble; `scicalc_audit_reset(name)` clears it. Capture is keyed to an environment variable, so normal interactive work logs nothing and a self-auditing script cannot recurse.
