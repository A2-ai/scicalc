...
.write_csv_with_hash <- function(data, csv_path, ..., algo = "blake3") {
  ...
  cat("\n")
  log_audit_event("write", fn = "write_file_with_hash", file = basename(csv_path), hash = hash, algo = algo)
}
...
.write_parquet_with_hash <- function(data, parquet_path, ..., algo = "blake3") {
  ...
  cat("\n")
  log_audit_event("write", fn = "write_file_with_hash", file = basename(parquet_path), hash = hash, algo = algo)
}
